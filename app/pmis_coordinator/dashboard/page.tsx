...
function agingBadge(dateStr) {
  ...
  const days = Math.floor((new Date(2026,4,24) - d) / 86400000);
...
function ValidateModal({ entries, selectedRef, onSelect, onValidate, onClose }) {
  const item = selectedRef ? entries.find((e) => e.ref === selectedRef) : null;
  const actionable = entries.filter((e) => e.status === "Pending" || e.status === "In Progress");
          ...
          {item ? (
            ...
            <div>
              ...
              <div className="space-y-1.5 max-h-52 overflow-y-auto">
                {actionable.length > 0 ? actionable.map((e) => (
    ...
    <ModalBackdrop onClose={onClose}>
      <div className="bg-white border border-gray-200 rounded-xl w-[460px] max-w-[95vw] shadow-lg overflow-hidden">
        ...
        <div className="p-4 space-y-3">
          ...
          >
            <input id="pmis-file-input" type="file" accept=".xlsx,.csv,.pdf" className="hidden" onChange={(e) => setFile(e.target.files[0])} />
            <Upload className={`w-5 h-5 mx-auto mb-1.5 ${dragging ? "text-blue-500" : "text-gray-300"}`} />
            {file
              ? <p className="text-[12px] text-blue-700 font-medium">{file.name}</p>
              : <>
...
/* ── Sync PMIS Modal ── */
function SyncModal({ entries, onClose, showToast }) {
  ...
  const discrepant = entries.filter((e) => e.status === "Discrepancy").length;
  const pending    = entries.filter((e) => e.status === "Pending" || e.status === "In Progress").length;
...
function SearchModal({ entries, onClose }) {
...
function BoardModal({ entries, onClose, onOpenModal, onSelectRef }) {
  ...
  function handleAction(type, ref) {
    onSelectRef(ref);
    onClose();
    onOpenModal(type);
  }
...
export default function PMISCoordinatorDashboard() {
  ...
  const stats = [
    { label: "For PMIS Validation", value: entries.filter((e) => e.status === "Pending").length,                                 sub: "data entries pending",  icon: Database,    color: "blue"  },
    { label: "In Progress",         value: entries.filter((e) => e.status === "In Progress").length,                             sub: "currently reviewing",   icon: Clock,       color: "amber" },
    { label: "Validated",           value: entries.filter((e) => e.status === "Validated").length,                               sub: "entries approved",      icon: CheckCircle, color: "green" },
    { label: "Discrepancies",       value: entries.filter((e) => e.status === "Discrepancy").length,                             sub: "data mismatches found", icon: AlertCircle, color: "red"   },
  ];

  const discrepantItems = entries.filter((e) => e.status === "Discrepancy");
